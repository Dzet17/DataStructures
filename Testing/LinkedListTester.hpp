//
//  LinkedListTester.hpp
//  DataStructures
//
//  Created by Zettler, Dylan on 2/22/18.
//  Copyright © 2018 CTEC. All rights reserved.
//

#ifndef LinkedListTester_hpp
#define LinkedListTester_hpp

#include <iostream>
#include "/Users/dzet4165/Documents/Data Structures/DataStructures/DataStructures/Model/Nodes/LinearNode.hpp"
#include "/Users/dzet4165/Documents/Data Structures/DataStructures/DataStructures/Controller/FileController.hpp"
#include "/Users/dzet4165/Documents/Data Structures/DataStructures/DataStructures/Controller/Tools/Timer.hpp"
#include "/Users/dzet4165/Documents/Data Structures/DataStructures/DataStructures/Data/CrimeData.hpp"

class LinkedListTester
{
public:
    void testListBasics();
    void testListWithData();
};

void LinkedListTester :: testListBasics()
{
    Linkedlist<int> numbers;
    numbers.add(9)
    cout << numbers.getFront()->getData() << " Is is at the front of the list and should be 9" << end;
    cout  << numbers.getEnd()->getData() << " Is is at the end of the list and should be 9" << end;
    
    numbers.add(0);
    cout << numbers.getFront()->getData() << " Is is at the front of the list and should be 0" << end;
    cout  << numbers.getEnd()->getData() << " Is is at the end of the list and should be 0" << end;
    
    numbers.addAtIndex(1,2);
    numbers.add(324);
    
    cout << numbers.getFront()->getData() << " Is is at the front of the list and should be 9" << end;
    cout  << numbers.getEnd()->getData() << " Is is at the end of the list and should be 324" << end;
    
    cout  <<"This loop should print 4 lines"<< endl
    for (int index = 0; index < numbers.getSize(); index++)
    {
        cout << numbers.getFromIndex(index) << endl;
    }
    
    numbers.remove(0);
    numbers.add(32567);
    numbers.addAtIndex(0, 2312);
    
    cout << numbers.getFront()->getData() << " Is is at the front of the list and should be 2312" << end;
    cout << numbers.getEnd()->getData() << " Is is at the end of the list and should be 32567" << end;
    
}

void LinkedListTester :: testListWithData()
{
    Timer listTimer;
    
    listTimer.startTimer();
    LinkedList<CrimeData> crimes = FileController :: readDataoList("/Users/dzet4165/Documents/Data Structures/DataStructures/DataStructures/Data/crime.csv");
    listTimer.stopTimer();
    cout << "This is how long it took to read the structure into our custom data structure" << endl;
    listTimer.displayInformation();
    
    listTimer.resetTimer();
    cout << "Here is how long it takes to access a random data value" << endl;
    listTimer.startTimer();
    int randomLocation = (randd() * rand()) % crimes.getSize();
    cout << "The random index is " << randomLocation << endl;
}
#endif /* LinkedListTester_hpp */
